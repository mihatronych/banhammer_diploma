...
export class PostController extends BasicController<PostRepository> {
    constructor(connection:Connection) {
        super(connection, PostRepository, false);
    ...
    async getAllMerged(req: Request, res: Response, next: any): Promise<Response> {
        let merged_entities = []
        let entities = await (this._repository as BasicRepository<Post>).find()
        entities.map(post =>
        {
            let temp = {}
            temp["id"] = post.id
            temp["text"] = post.text
            temp["publication_date"] = post.publication_date
            temp["pictures"] = post.pictures
            temp["message_topics"] = post.message_topics
            temp["preprocessed_text"] = post.preprocessed_text
